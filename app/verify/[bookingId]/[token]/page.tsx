...
export default async function VerifyPage({ params }: { params: Promise<{ bookingId: string; token: string }> }) {
  ...
  const valid = booking && booking.status === 'PENDING' && booking.expiresAt && booking.expiresAt > now
  ...
  return (
    <main className="bg-gray-50 flex items-center justify-center px-4 py-16">
      <div className="max-w-md w-full bg-white p-8 rounded-lg shadow space-y-4 text-gray-900">
        <h1 className="text-2xl font-bold">Buchung bestätigen</h1>
        {!booking ? (
          <Notice tone="error">
          ...
          <>
            <Notice tone="error">Die Reservierung ist abgelaufen, der Tisch ist wieder frei.</Notice>
            <p className="text-sm"><Link href={`/${booking.event.slug}`} className="text-blue-700 underline">Neu buchen</Link></p>
          </>
        ) : (
          <>
            <dl className="text-sm grid grid-cols-[auto_1fr] gap-x-4 gap-y-1">
              ...
              <dt className="text-gray-600">Tisch</dt><dd>{table?.label}</dd>
            ...
            <p className="text-sm text-gray-700">
              Reserviert bis {formatDateTime(booking.expiresAt!, booking.event.timezone)}. Mit dem Klick bestätigst du deine
              E-Mail-Adresse, und die Buchung gilt.
            </p>
            <ConfirmButton bookingId={booking.id} token={token} />
          </>
        )}
      </div>
    </main>
